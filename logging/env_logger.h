...
class EnvLogger : public Logger {
 public:
  EnvLogger(std::unique_ptr<FSWritableFile>&& writable_file, const std::string& fname,
            const EnvOptions& options, Env* env,
            InfoLogLevel log_level = InfoLogLevel::ERROR_LEVEL,
            const std::shared_ptr<Statistics>& stats = nullptr)
      : Logger(log_level),
        env_(env),
        ...
        statistics_(stats),
        bg_stat_cv_(&bg_stat_mutex_),
        bg_stopped_(false) {
    if (statistics_) {
      bg_thread_.reset(new port::Thread(&EnvLogger::BgWork, this));
    }
  }

  void SetStatistics(const std::shared_ptr<Statistics>& stats) {
    if (!stats) {
      return;
    }
    if (statistics_) {
  ...
  ~EnvLogger() {
    if (bg_thread_) {
      {
        MutexLock l(&bg_stat_mutex_);
        bg_stopped_ = true;
        bg_stat_cv_.SignalAll();
      }
      bg_thread_->join();
    }
    if (!closed_) {
 ...
 private:
  ...
  void BgWork() {
    uint64_t last_write_ops_count =
        statistics_->getTickerCount(NUMBER_KEYS_WRITTEN);
    uint64_t last_read_ops_count =
        statistics_->getTickerCount(NUMBER_KEYS_READ);
    ...
    while (true) {
      {
        ...
        // Wait for 1 second
        bg_stat_cv_.TimedWait(clock_->NowMicros() + 1000 * 1000);
        if (bg_stopped_) {
          return;
        }
      }

      uint64_t current_write_ops_count =
          statistics_->getTickerCount(NUMBER_KEYS_WRITTEN);
      uint64_t current_read_ops_count =
          statistics_->getTickerCount(NUMBER_KEYS_READ);
      uint64_t write_ops_in_interval =
          current_write_ops_count - last_write_ops_count;
      uint64_t read_ops_in_interval =
          current_read_ops_count - last_read_ops_count;
      uint64_t ops_in_interval = read_ops_in_interval + write_ops_in_interval;
      last_write_ops_count = current_write_ops_count;
      last_read_ops_count = current_read_ops_count;

      char throughput_buffer[200];
      port::TimeVal now_tv;
      port::GetTimeOfDay(&now_tv, nullptr);
      const time_t seconds = now_tv.tv_sec;
      struct tm t;
      ...
      int len = snprintf(
          throughput_buffer, sizeof(throughput_buffer), 
          "%04d/%02d/%02d-%02d:%02d:%02d.%06d %llu ops: %llu\n",
          t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min,
          t.tm_sec, static_cast<int>(now_tv.tv_usec),
          static_cast<long long unsigned int>(thread_id),
          static_cast<unsigned long long>(ops_in_interval));

      {
        FileOpGuard guard(*this);
        file_.Append(Slice(throughput_buffer, len)).PermitUncheckedError();
        file_.reset_seen_error();
        flush_pending_ = true;
        const uint64_t now_micros = clock_->NowMicros();
        if (now_micros - last_flush_micros_ >=
 ...
 private:
  ...
  port::Mutex bg_stat_mutex_;
  port::CondVar bg_stat_cv_;
  bool bg_stopped_;
};
