...
void AutoRollLogger::SetStatistics(const std::shared_ptr<Statistics>& stats) {
  if (!stats) {
    return;
  }
  MutexLock l(&mutex_);
...
void AutoRollLogger::ApplyStatsToLogger() {
  if (!logger_ || !statistics_) {
    return;
  }
  auto* env_logger = dynamic_cast<EnvLogger*>(logger_.get());
  if (env_logger != nullptr) {
    env_logger->SetStatistics(statistics_);
  }
}
...
Status CreateLoggerFromOptions(const std::string& dbname,
  ...
  if (options.log_file_time_to_roll > 0 || options.max_log_file_size > 0) {
    ...
    if (!s.ok()) {
      ...
      if (options.statistics) {
        result->SetStatistics(options.statistics);
      }
    }
    return s;
  }
  ...
  if (s.ok() && logger->get() != nullptr) {
    if (options.statistics) {
      if (auto* env_logger = dynamic_cast<EnvLogger*>(logger->get())) {
        env_logger->SetStatistics(options.statistics);
      }
    }
