...
log_file_dir="/home/eros/forRTC/oadoc/result/motivation-${current_time}/"
mkdir -p "$log_file_dir"
mkdir -p "$output_dir"
summary_log="${output_dir}${bench_nums}m_cpu_mem_totals_${current_time}.csv"
echo "elapsed_seconds,use_rtc,uni,num,thread,memtable_mb,sstable_mb,config,round,cpu_percent,user_seconds,sys_seconds,max_rss_kb" > "$summary_log"
...
for num in "${bench_nums[@]}"; do
  for uni in "${unis[@]}"; do
    for cfg in "${configs[@]}"; do
      eval "$cfg"
      for memtable_mb in "${memtable_sizes_mb[@]}"; do
        memtable_bytes=$(( memtable_mb * 1024 * 1024 ))
        for sstable_mb in "${sstable_sizes_mb[@]}"; do
          sstable_bytes=$(( sstable_mb * 1024 * 1024 ))
          for thread in "${threads[@]}"; do
            for round in $(seq 1 $number_of_runs); do
              for rtc in "${use_rtc[@]}"; do
                log_file=${log_file_dir}out_${num}_${name}_uni${uni}_mem${memtable_mb}m_sst${sstable_mb}m_rtc${rtc}_thread${thread}_round${round}.log
                bench_file="${bench_db_base}${num}_${name}_uni${uni}_mem${memtable_mb}m_sst${sstable_mb}m_rtc${rtc}_thread${thread}_round${round}"
                ...
                const_params=" \
                  ...
                  --write_buffer_size=$memtable_bytes \
                  --target_file_size_base=$memtable_bytes \
                  --ARK_enable=${ark} \
                  --TEA_enable=${tea} \
                  --FEA_enable=${fea} \
                  --histogram " 
                ...
                echo "[INFO] Running round $round, uni=$uni, memtable=${memtable_mb}MB, sstable=${sstable_mb}MB, rtc=$rtc, config=${name}, threads=$thread" | tee -a "$log_file"
                ...
                # 모니터링 로그 파일 prefix
                monitor_prefix="${log_file_dir}monitor_${num}_${name}_uni${uni}_mem${memtable_mb}m_sst${sstable_mb}m_rtc${rtc}_round${round}"
                ...
                echo "${elapsed},${rtc},${uni},${num},${thread},${memtable_mb},${sstable_mb},${name},${round},${cpu_percent},${user_sec},${sys_sec},${max_rss}" >> "$summary_log"
                if [[ -d "$bench_file" ]]; then
                  log_dump_prefix="${log_file_dir}db_log_${num}_${name}_uni${uni}_mem${memtable_mb}m_sst${sstable_mb}m_rtc${rtc}_thread${thread}_round${round}"
